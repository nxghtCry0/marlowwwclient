...
public final class GlassyDropdown<T> extends CompatAbstractWidget {
    ...
    protected void renderWidget(DrawContext DrawContext, int mouseX, int mouseY, float partialTick) {
        ...
        WwUiStyle.drawDropdownCaret(DrawContext, x + w - 16, y, 16, h, this.active);
    ...
    public void renderOverlay(DrawContext DrawContext, int mouseX, int mouseY) {
        ...
        DrawContext.fill(x, menuY, x + w, menuY + menuH, GlassyTheme.PANEL_BG);
        DrawContext.renderOutline(x, menuY, w, menuH, GlassyTheme.PANEL_BORDER_STRONG);
        ...
        for (int i = 0; i < this.options.size(); i++) {
            ...
            if (isHovered) {
                this.hoveredIndex = i;
                DrawContext.fill(x + 1, itemY, x + w - 1, itemY + itemH, GlassyTheme.ROW_HOVER_BG);
            }
            ...
            DrawContext.drawString(font, optMsg, textX, itemY + (itemH - 9) / 2, optText, false);
        }
    }
